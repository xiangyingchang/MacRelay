        ...
        client.onEvent = { (event: CodexAppServerEvent) in
            ...
            case let .response(id, result, error):
                ...
                if m == "initialize" {
                    ...
                    do { let tid = try client.request(method: "thread/start", params: ["cwd": cwd, "sandbox": "read-only", "approvalPolicy": "on-request", "sessionStartSource": "startup"]); remember(tid, method: "thread/start") } catch { finish("thread/start failed: \(error)") }
                } else if m == "thread/start" {
                    do { let tid = try client.request(method: "turn/start", params: ["threadId": snapshot.threadID ?? "", "input": [["type": "text", "text": prompt]], "approvalPolicy": "on-request"]); remember(tid, method: "turn/start") } catch { finish("turn/start failed: \(error)") }
