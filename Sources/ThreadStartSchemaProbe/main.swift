...
client.onEvent = { event in
    ...
    case let .exit(code, _):
        ...
        if !didFinish {
            finish("app-server exited before thread/start result")
        }
    }
}
...
do {
    ...
    let initializeID = try client.request(method: "initialize", params: [
        "clientInfo": [
            "name": "agent-client-thread-start-schema-probe",
            "title": "Agent Client Thread Start Schema Probe",
            "version": "0.1.0"
        ],
        "capabilities": [
...
if semaphore.wait(timeout: .now() + .seconds(30)) == .timedOut {
    failure = "timed out waiting for thread/start"
}

client.stop()

if let failure {
    print("ThreadStartSchemaProbe failed: \(failure)")
    exit(1)
}
