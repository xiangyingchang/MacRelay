...
@main
struct CodexAppServerInitProbe {
    static func main() {
        #if os(macOS)
        ...
        client.onEvent = { (event: CodexAppServerEvent) in
            ...
            case let .response(id, result, error):
                if let error {
                    print("response error id=\(id) error=\(error)")
                    failure = "initialize failed: \(error)"
                } else {
                    print("response id=\(id) keys=\((result ?? [:]).keys.sorted())")
                }
                semaphore.signal()
            ...
            case let .exit(code, _):
                ...
                if failure == nil { failure = "app-server exited before response" }
                semaphore.signal()
            default: break
            }
        }
